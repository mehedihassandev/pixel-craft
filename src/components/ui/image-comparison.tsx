...
export const ImageComparison: React.FC<ImageComparisonProps> = ({
  ...
  const [activeView, setActiveView] = useState<'original' | 'processed' | 'comparison'>(
    'comparison'
  );
  ...
  const copyUrlToClipboard = async (url: string, imageType: 'original' | 'processed') => {
    try {
      ...
      <Card className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm shadow-lg">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              variant={activeView === 'processed' ? 'default' : 'outline'}
              size="sm"
          ...
          {type === 'resize' && originalDimensions && targetDimensions && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center p-3 bg-muted rounded-lg">
                ...
                <div className="font-semibold">{calculateCompressionRatio()}</div>
              </div>
              <div className="text-center p-3 bg-muted rounded-lg">
                <div className="text-sm text-muted-foreground">Quality</div>
                <div className="font-semibold">{imageQuality}%</div>
              </div>
            </div>
          )}
                ...
                onClick={() => {
                  const filename =
                    type === 'background-remove'
                      ? `background-removed.${imageFormat || 'png'}`
            ...
            {originalImageUrl && (
              <>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => copyUrlToClipboard(originalImageUrl, 'original')}
                >
                  <Copy className="h-4 w-4 mr-1" />
                  Copy Original URL
                </Button>
                <Button variant="outline" size="sm" onClick={() => openInNewTab(originalImageUrl)}>
            ...
            {processedImageUrl && (
              <>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => copyUrlToClipboard(processedImageUrl, 'processed')}
                >
                  <Copy className="h-4 w-4 mr-1" />
                  Copy Processed URL
                </Button>
                <Button variant="outline" size="sm" onClick={() => openInNewTab(processedImageUrl)}>
                  <ExternalLink className="h-4 w-4 mr-1" />
                  Open Processed
                </Button>
              </>
      ...
      <Card className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm shadow-lg">
        <CardContent className="p-6">
          <div className="relative rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-900">
            ...
            {activeView === 'comparison' && originalImageUrl && processedImageUrl && (
              <div
              ...
              >
                ...
                >
                  {/* Drag handle */}
                  <div
                    className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-full shadow-lg border-2 flex items-center justify-center transition-all duration-200 ${
                      isDragging
                  ...
                  >
                    <ArrowLeftRight
                      className={`w-4 h-4 ${isDragging ? 'text-white' : 'text-gray-600'}`}
                    />
                  </div>
                </div>
              </div>
            ...
            {activeView === 'comparison' && originalImageUrl && processedImageUrl && (
              <>
                ...
                <div className="absolute top-2 right-2 z-40">
                  <Badge variant="secondary">{getProcessedLabel()}</Badge>
                </div>
              </>
            )}
