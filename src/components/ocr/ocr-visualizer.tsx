...
export const OcrVisualizer = ({ imageUrl, words, blocks, confidence }: OcrVisualizerProps) => {
  ...
  }, [imageLoaded, showWords, showBlocks, confidenceThreshold, highlightColor, words, blocks]);
    ...
    if (showBlocks && blocks) {
      ...
      blocks
        ...
        .forEach(block => {
          ...
          ctx.font = '14px Arial';
          ctx.fillText(`Block: ${Math.round(block.confidence)}%`, x, y - 5);
        });
    }
  };
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-2">
                <Switch id="showWords" checked={showWords} onCheckedChange={setShowWords} />
                <Label htmlFor="showWords">Show Words</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Switch id="showBlocks" checked={showBlocks} onCheckedChange={setShowBlocks} />
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <span className="text-sm text-gray-600">{highlightColor}</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t">
            <div className="text-center">
              ...
              <div className="text-sm text-gray-600">Words Shown</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-red-600">
                {stats.blocksCount}/{stats.totalBlocks}
              </div>
              <div className="text-sm text-gray-600">Blocks Shown</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {stats.avgWordConfidence.toFixed(1)}%
              </div>
              <div className="text-sm text-gray-600">Avg Word Confidence</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">
                {stats.avgBlockConfidence.toFixed(1)}%
              </div>
              <div className="text-sm text-gray-600">Avg Block Confidence</div>
            </div>
          </div>
        </CardContent>
      </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {words
                  ...
                  .map((word, index) => (
                    ...
                    >
                      <span className="text-sm truncate">{word.text}</span>
                      <Badge
                        variant={
                          word.confidence >= 80
                            ? 'default'
