...
export const OcrForm = () => {
  ...
  const captureFromCamera = useCallback(async () => {
    try {
      ...
      toast({
        ...
        description: 'Please allow camera access or select an image file',
        variant: 'destructive',
      });
    }
  ...
  const initializeWorker = useCallback(async () => {
    ...
    const newWorker = await Tesseract.createWorker(watchedValues.language, watchedValues.oem, {
      logger: m => {
  ...
  const autoProcessOcr = useCallback(
    async (file: File) => {
      const currentData = {
      ...
      try {
        ...
        if (currentData.outputFormat === 'json') {
          if (ocrResult.words) {
            processedResult.words = ocrResult.words.map((word: any) => ({
              text: word.text,
          ...
          if (ocrResult.blocks) {
            processedResult.blocks = ocrResult.blocks.map((block: any) => ({
              text: block.text,
        ...
        if (currentData.removeExtra) {
          processedResult.text = processedResult.text.replace(/\s+/g, ' ').trim();
        }
        ...
        toast({
          ...
          description: `Extracted text with ${processedResult.confidence.toFixed(1)}% confidence`,
        });
      } catch (error) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={value =>
                        setValue('outputFormat', value as 'text' | 'json' | 'hocr')
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="json">JSON (with coordinates)</SelectItem>
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      checked={watchedValues.preserveLineBreaks}
                      onCheckedChange={checked => setValue('preserveLineBreaks', checked)}
                    />
                    <Label htmlFor="preserveLineBreaks">Preserve line breaks</Label>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={checked => setValue('removeExtra', checked)}
                    />
                    <Label htmlFor="removeExtra">Remove extra whitespace</Label>
                  </div>
                </div>

                {/* Confidence Filter */}
                <div className="space-y-2">
                  <Label>Minimum Confidence: {watchedValues.confidence}%</Label>
                  <Slider
                    value={[watchedValues.confidence]}
                    onValueChange={value => setValue('confidence', value[0])}
                    max={100}
                    min={0}
                    step={5}
                    className="w-full"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={value => setValue('psm', parseInt(value))}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={value => setValue('oem', parseInt(value))}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
          ...
          <CardContent>
            <Tabs defaultValue="text" className="space-y-4">
              <TabsList>
                ...
                {result.words && <TabsTrigger value="json">JSON</TabsTrigger>}
                {result.hocr && <TabsTrigger value="hocr">hOCR</TabsTrigger>}
              ...
              <TabsContent value="text" className="space-y-3">
                ...
                <Textarea value={result.text} readOnly rows={10} className="resize-none" />
              </TabsContent>
                ...
                <TabsContent value="hocr" className="space-y-3">
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => copyToClipboard(result.hocr || '')}
                      className="flex items-center gap-2"
                    >
                      <Copy className="h-4 w-4" />
                      Copy hOCR
